    ...
    if (!mutex.isLocked()) {
      ...
      try {
        ...
        if (refreshResult.data && isTokens(refreshResult.data)) {
          ...
          //initiate — это низкоуровневый способ вручную запустить выполнение запроса (query)
          // или мутации (mutation) вне хуков.
          // Обычно мы используем хуки типа useGetUsersQuery(), но initiate нужен, когда логика
          // находится вне React-компонентов или требует особого контроля.
        }
      } finally {
...
//todo - baseQueryWithReauth - проходит реавторизация